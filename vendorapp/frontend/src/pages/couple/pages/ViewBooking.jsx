...
import { addToInvoice, underControl } from '../../../redux/userSlice';
...
const ViewBooking = () => {
    ...
    const { currentUser, currentRole, serviceDetails, loading, status, error, responseReview, responseDetails } = useSelector(state => state.user);
    ...
    const handleSubmit = (event) => {
        ...
        if (rating === 0) {
            ...
            dispatch(updateStuff(fields, serviceID, "addReview"));
            setRating(0);
            setComment('');
        }
    };
    ...
    useEffect(() => {
        if (status === "updated") {
            ...
            setMessage("You have already submitted a review for this service.");
            setShowPopup(true);
        } else if (error) {
            setMessage("Network Error");
            setShowPopup(true);
        }
    }, [dispatch, responseReview, serviceID, status, error]);
