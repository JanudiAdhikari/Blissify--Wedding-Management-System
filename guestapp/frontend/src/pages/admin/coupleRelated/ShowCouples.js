...
const ShowCouples = () => {
    ...
    const { couplesList, loading, error, response } = useSelector((state) => state.couple);
    ...
    if (loading) {
        ...
        return (
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', marginTop: '16px' }}>
                <GreenButton variant="contained" onClick={() => navigate("/Admin/couples/choosetable")}>
                    Add Couple
                </GreenButton>
            </Box>
        );
    ...
    const actions = [
        {
            icon: <PersonAddAlt1Icon color="primary" />, name: 'Add New Couple',
            action: () => navigate("/Admin/couples/choosetable")
        },
        {
            icon: <PersonRemoveIcon color="error" />, name: 'Delete All Couples',
                            ...
                            .map((row) => {
                                return (
                                    <StyledTableRow hover role="checkbox" tabIndex={-1} key={row.id}>
                                                    ...
                                                    <StyledTableCell key={column.id} align={column.align}>
                                                        {value ? (
                                                            ...
                                                            <Button variant="contained"
                                                                onClick={() => {
                                                                    navigate(`/Admin/couples/choosenote/${row.teachStableID}/${row.id}`)
                                                                }}>
                                                                Add Note
                                                            </Button>
                                                        )}
                                        ...
                                        <StyledTableCell align="center">
                                            <IconButton onClick={() => deleteHandler(row.id, "Couple")}>
                                                <PersonRemoveIcon color="error" />
                                            </IconButton>
                                            <BlueButton variant="contained"
                                                onClick={() => navigate("/Admin/couples/couple/" + row.id)}>
                                                View
                                            </BlueButton>
                                        </StyledTableCell>
                                    </StyledTableRow>
