...
const OutForDeliverySection = () => {
    ...
    const { currentUser, specificServiceData, responseSpecificServices } = useSelector(state => state.user);

    useEffect(() => {
        dispatch(getSpecificServices(currentUser._id, "getBookingedServicesByVendor"));
    ...
    const servicesRows = Array.isArray(specificServiceData) && specificServiceData.length > 0
        ...
        <>
            {responseSpecificServices ?
                ...
                <>
                    ...
                    <TableTemplate buttonHaver={ServicesButtonHaver} columns={servicesColumns} rows={servicesRows} />
                </>
            }
        </>
    )
